...
module.exports = class Issue {
    constructor({key, id, fields, transitions, jira}) {
    ...
    comment({comment, hidden = false}) {
        ...
        this.jira.issue.addComment(commentData, (error, data) => {
            (error) ? log.error(error) : log.trace('Comment added');
        });
    ...
    deleteAttachment(attachmentId) {
        this.jira.attachment.deleteAttachment({attachmentId: attachmentId}, (error, data) => {
            (error) ? log.error(error) : log.trace('Attachment deleted');
        })
    }
